...
syncDir_dir = [{'type': 'temp', 'disk':'/media/USBdisk', 'dir': 'syncDir'},
            {'type': 'temp', 'disk':'/home/yisp','dir': 'syncDir'},
          ]
...
def args_prompt(jobs):
    ...
    sel_func = raw_input("(default 0) >> ")
    if sel_func == '1':
        print("\nWARNING:sync up is selected. Suggest to search and check the diff first!\n")
        mode = raw_input("input Y/y to confirm (default n) >> ")
        if mode == 'y' or mode == 'Y' :
            ulUpdate_enable =1
